...
public class CommandItems extends JavaPlugin {
	...
	public void onEnable() {
		...
		this.commandManager
			.getCommandContexts()
			.registerContext(ItemDefinition.class, context -> {
		...
		this.commandManager
			...
			.registerCompletion("itemdefs", context -> this.configManager
				.getConfig()
				.getItems()
				.keySet()
				.stream()
				.filter(key -> key.toLowerCase().startsWith(context.getInput().toLowerCase()).collect(Collectors.toList()))
			);

		this.commandManager
			.getCommandCompletions()
			.registerCompletion("itemparams", context -> {
